        ...
        const apiTutorsWithCache = apiTutors.map(tutor => {
          ...
          return {
              ...
              petIds: tutor.petIds && tutor.petIds.length > 0 
                ? tutor.petIds 
                : (localData?.petIds || []),
              // Preservar imagens, documentos e foto do cache local se API não retornar
              imagens: tutor.imagens && tutor.imagens.length > 0
                ? tutor.imagens
                : (localData?.imagens || []),
      ...
      try {
        ...
        const mergedTutor = {
          ...
          petIds: response.data.petIds && response.data.petIds.length > 0 
            ...
            : (localData?.petIds || []),
          // Preservar imagens, documentos e foto do cache local se API não retornar
          imagens: response.data.imagens && response.data.imagens.length > 0
  ...
  async updateTutor(id: string, tutor: Partial<Tutor>): Promise<Tutor> {
    ...
    try {
      ...
      if (baseTutor) {
        ...
        const hasImagens = Object.prototype.hasOwnProperty.call(tutor, 'imagens');
        ...
        if (!hasImagens) payload.imagens = baseTutor.imagens;
      ...
      try {
        ...
        const mergedTutor = {
          ...
          imagens: hasImagens
            ? updatedTutorLocal.imagens
            : (response.data.imagens && response.data.imagens.length > 0
              ? response.data.imagens
              : updatedTutorLocal.imagens),
          // Preservar documentos do cache local caso a API não retorne
          documentos: hasDocumentos
            ...
            : (response.data.documentos && response.data.documentos.length > 0
              ...
              : updatedTutorLocal.documentos),
          // Priorizar foto local (mesma logica usada em pets)
          foto: this.resolveFoto(hasFoto ? updatedTutorLocal.foto : fotoOverride ?? updatedTutorLocal.foto, response.data.foto),
        };
