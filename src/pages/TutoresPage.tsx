        ...
        {!loading && hasLoaded && totalTutores > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTutors.map((tutor: Tutor) => (
              <Link key={tutor.id} to={`/tutores/${tutor.id}`} className="group bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-purple-200 hover:-translate-y-1 block">
                {/* Content */}
                <div className="p-6">
                  {/* Name */}
                  <h3 className="text-xl font-bold text-gray-900 mb-4 line-clamp-2 group-hover:text-purple-600 transition">
                    ...
                    {tutor.endereco && (
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-semibold text-gray-900 line-clamp-2">{tutor.endereco}</p>
                        </div>
                      </div>
                    )}
                  </div>

                  {/* Footer com botão Detalhes, igual ao PetCard */}
                  <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                    <span className="flex items-center gap-1 group-hover:text-blue-600 transition font-semibold text-gray-700">
                      <span className="text-sm">Detalhes</span>
                      <svg className="w-5 h-5 text-blue-600 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                    </span>
                  </div>
                </div>
              </Link>
